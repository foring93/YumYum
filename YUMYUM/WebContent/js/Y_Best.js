$(document).ready(function(){
	// # 처음 접속할 때 베스트순으로 리스트 출력 
	show();
	
	function show (city, subcity, kind) {
		$.ajax({
			type : "get",
			url : 'place.best',
			data : {'place' : city, 'subplace' : subcity, 'kind' : kind},
			dataType : 'json',
			success : function (result) {
				...
				if (result == null || result == "") {
					...
					$.each(result, function (index, item) {
						...
						output += '			<img src = "../img/' + item.mp_img_url + '" class = "yh_BestImg"><br>';
	...
	$('#sel_city').change(function() {
		...
		if ($('#sel_city').val() == '') {
			...
			$('.yh_Y_Best').html('');
			show($(this).val(), $('#sel_subcity').val(), $('#sel_food').val());	
		}
	...
	$('#sel_subcity').change(function() {
		$('.yh_Y_Best').html('');	
		show($('#sel_city').val(), $(this).val(), $('#sel_food').val());
	});
	...
	$('#sel_food').change(function(){
		$('.yh_Y_Best').html('');	
		show($('#sel_city').val(), $('#sel_subcity').val(), $(this).val());
	});
