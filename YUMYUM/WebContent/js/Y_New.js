...
function go(page) {
	show($('#sel_city').val(), $('#sel_subcity').val(), $('#sel_food').val(), page);
}
...
function setPaging(href, digit) {
	...
	anchor = "<a class = 'page-link " + gray + "'" + href + ">" + digit + "</a>";
	output += anchor;
}

//# 맛집 찾아오기 
function show(city, subcity, kind, page) {
	$.ajax({
		type : "get",
		url : "place.new",
		data : {'place' : city, 'subplace' : subcity, 'kind' : kind, 'page' : page},
		dataType : 'json',
		cache : false,
		success : function(data) {
			...
			if (data.listcount > 0) {
				...
				$(data.placelist).each(function(index, item) {
					...
					output += '	<td>';
					output += '		<a href = "place.move?mp_no=' + item.mp_no +'" class = "yh_move_place">';
					output += '			<img src = "../img/' + item.mp_img_url + '" class = "yh_NewImg"><br>';
...
$(document).ready(function(){
	// # 처음 접속할 때 최신순으로 리스트 출력
	show($('#sel_city').val(), $('#sel_subcity').val(), $('#sel_food').val(), 1);
	...
	$('#sel_city').change(function() {
		...
		if ($('#sel_city').val() == '') {
			...
			$('.yh_Y_New').html('');			// 신규 맛집 전체를 가져오기 위해 테이블을 비운다.
			show($(this).val(), $('#sel_subcity').val(), $('#sel_food').val(), 1);	
		}
		
		// 서울 선택 시 서울의 세부 지역 추가
		if ($('#sel_city').val() == '서울') {
			$('#sel_subcity').html('');				// 세부 지역의 값을 지우고
			$('#sel_subcity').append(seoul());		// 서울에 있는 세부 지역을 추가한다.
			$('.yh_Y_New').html('');				// 서울에 있는 신규 맛집을 가져오기 위해 테이블을 비운다.
			show($(this).val(), $('#sel_subcity').val(), $('#sel_food').val(), 1);	
		}
		...
		if ($('#sel_city').val() == '경기') {
			$('#sel_subcity').html('');				// 세부 지역의 값을 지우고
			$('#sel_subcity').append(gyeonggi());	// 경기에 있는 세부 지역을 추가한다.
			$('.yh_Y_New').html('');				// 경기에 있는 신규 맛집을 가져오기 위해 테이블을 비운다.
			show($(this).val(), $('#sel_subcity').val(), $('#sel_food').val(), 1);	
		}
		...
		if ($('#sel_city').val() == '인천') {
			$('#sel_subcity').html('');				// 세부 지역의 값을 지우고
			$('#sel_subcity').append(incheon());	// 인천에 있는 세부 지역을 추가한다.
			$('.yh_Y_New').html('');				// 인천에 있는 신규 맛집을 가져오기 위해 테이블을 비운다.
			show($(this).val(), $('#sel_subcity').val(), $('#sel_food').val(), 1);	
		}
		...
		if ($('#sel_city').val() == '대전') {
			$('#sel_subcity').html('');				// 세부 지역의 값을 지우고
			$('#sel_subcity').append(daejeon());	// 대전에 있는 세부 지역을 추가한다.
			$('.yh_Y_New').html('');				// 대전에 있는 신규 맛집을 가져오기 위해 테이블을 비운다.
			show($(this).val(), $('#sel_subcity').val(), $('#sel_food').val(), 1);	
		}
		...
		if ($('#sel_city').val() == '대구') {
			$('#sel_subcity').html('');				// 세부 지역의 값을 지우고
			$('#sel_subcity').append(daegu());		// 대구에 있는 세부 지역을 추가한다.
			$('.yh_Y_New').html('');				// 대구에 있는 신규 맛집을 가져오기 위해 테이블을 비운다.
			show($(this).val(), $('#sel_subcity').val(), $('#sel_food').val(), 1);	
		}
		...
		if ($('#sel_city').val() == '부산') {
			$('#sel_subcity').html('');				// 세부 지역의 값을 지우고
			$('#sel_subcity').append(busan());		// 부산에 있는 세부 지역을 추가한다.
			$('.yh_Y_New').html('');				// 부산에 있는 신규 맛집을 가져오기 위해 테이블을 비운다.
			show($(this).val(), $('#sel_subcity').val(), $('#sel_food').val(), 1);	
		}
		...
		if ($('#sel_city').val() == '광주') {
			$('#sel_subcity').html('');				// 세부 지역의 값을 지우고
			$('#sel_subcity').append(gwangju());	// 광주에 있는 세부 지역을 추가한다.
			$('.yh_Y_New').html('');				// 광주에 있는 신규 맛집을 가져오기 위해 테이블을 비운다.
			show($(this).val(), $('#sel_subcity').val(), $('#sel_food').val(), 1);	
		}
		...
		if ($('#sel_city').val() == '제주') {
			$('#sel_subcity').html('');				// 세부 지역의 값을 지우고
			$('#sel_subcity').append(jeju());		// 제주에 있는 세부 지역을 추가한다.
			$('.yh_Y_New').html('');				// 제주에 있는 신규 맛집을 가져오기 위해 테이블을 비운다.
			show($(this).val(), $('#sel_subcity').val(), $('#sel_food').val(), 1);	
		}
	...
	$('#sel_subcity').change(function() {
		$('.yh_Y_New').html('');
		show($('#sel_city').val(), $(this).val(), $('#sel_food').val(), 1);
	});
	
	// 음식 종류 선택 시 
	$('#sel_food').change(function(){
		$('.yh_Y_New').html('');
		show($('#sel_city').val(), $('#sel_subcity').val(), $(this).val(), 1);
	});
