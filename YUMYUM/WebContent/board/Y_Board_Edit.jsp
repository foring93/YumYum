	...
	<body>
		...
		<div class="out_container">
			<div class="write_line">
				<form action="Y_Board_EditSuccess" method="post" id="writeform">
					<fieldset>
						<legend>
							<a href="#" class="HH_a">공지수정</a>
						</legend>
						<!-- <label for="board_no" class="HH_label">글번호</label> -->
						<input type="hidden" id="board_no" name="board_no" class="HH_input" readonly value="${board_data.board_no}">
						<br>
						<label for="writer" class="HH_label">작성자</label>
						<input type="text" id="writer" name="writer" readonly value="<%="운영자" %>" class="HH_input">
						<br>
						...
						<br>
						<input type="text" name="title" id="title" class="HH_input HH_title" placeholder="Title"
							   maxlength="100" value="${board_data.board_title}" required>
						<br>
						<label for="HH_content" class="HH_label">내용</label>
						<br>
						<textarea name="content" id="HH_content" rows="15" placeholder="Content" required>${board_data.board_content}</textarea>
						<button type="submit" class="HH_button">수정 완료</button>
		...
		<script>
			...
			if('${board_data.board_category}' == '공지')
			{
			...
			if(${board_data.board_is_all_time} == 1)
			{
				
			}
			else
			{
				$('#all_time').trigger('click');
				var exdate = '${board_data.board_expirate}'.split('-');
				var ex_year = exdate[0];
				var ex_month = exdate[1];
				var ex_day = exdate[2];
				$('#ex_year').val(ex_year);
				$('#ex_month').val(ex_month);
				$('#ex_day').val(ex_day);
			}
		</script>
		<footer>
